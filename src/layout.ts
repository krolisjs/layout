...
export function preset(style: Style, constraints: Constraints, type: Result['type'], root: Root, pc?: ComputedStyle, ps?: Style) {
  const res: any = {
    type,
    rects: type === 'box' ? null : [],
    x: constraints.cx,
  ...
  if (style.fontSize.u === Unit.INHERIT) {
    res.fontSize = pc?.fontSize || root.rem;
  }
  else {
    res.fontSize = calLength(style.fontSize, (pc?.fontSize || root.rem) * 100, root.rem, 0) || pc?.fontSize || root.rem;
  }
  ...
  ([
    ...
    if (v.u === Unit.INHERIT && ps) {
      res[k] = calLength(ps[k], constraints.pbw, root.rem, res.fontSize);
    }
    else {
      res[k] = calLength(style[k], constraints.pbw, root.rem, res.fontSize);
    }
  });
  ...
  ([
    ...
    if (v.u === Unit.INHERIT && ps) {
      ...
      res[k] = Math.max(0, calLength(style[k], constraints.pbw, root.rem, res.fontSize));
    }
  });
  ...
  ([
    ...
    if (k === 'lineHeight' && u === Unit.NUMBER) {
      ...
      res[k] = Math.max(0, calLength(style[k], pc?.lineHeight || 24, root.rem, res.fontSize));
    }
    else {
      res[k] = Math.max(0, calLength(style[k], constraints.pbw, root.rem, res.fontSize));
    }
  ...
  if (style.width.u !== Unit.AUTO) {
    res.w = Math.max(0, calLength(style.width, constraints.pbw, root.rem, res.fontSize));
  ...
  if (style.height.u !== Unit.AUTO && (constraints.pbh !== undefined || style.height.u !== Unit.PERCENT)) {
    res.h = Math.max(0, calLength(style.height, constraints.pbh || 0, root.rem, res.fontSize));
    if (style.boxSizing === BoxSizing.BORDER_BOX) {
...
export function block(style: Style, constraints: Constraints, root: Root, pc?: ComputedStyle, ps?: Style, res?: Box) {
  if (!res) {
    res = preset(style, constraints, 'box', root, pc, ps) as Box;
    res.type = 'box';
  }
  // 返回递归的供子节点使用
...
export function inline(style: Style, constraints: Constraints, root: Root, pc?: ComputedStyle, ps?: Style) {
  const res = preset(style, constraints, 'inline', root, pc, ps) as Inline;
...
export function text(style: Style, constraints: Constraints, content: string, root: Root, pc?: ComputedStyle, ps?: Style) {
  const measureText = getMeasureText();
  if (!measureText) {
    throw new Error('Text must be passed to the measureText method.');
  }
  const res = preset(style, constraints, 'text', root, pc, ps) as Text;
  // inline的上下margin无效
  res.marginTop = res.marginBottom = 0;
...
export function oofText(style: Style, constraints: Constraints, content: string, root: Root, pc: ComputedStyle, ps: Style) {
  const measureText = getMeasureText();
  if (!measureText) {
    throw new Error('Text must be passed to the measureText method.');
  }
